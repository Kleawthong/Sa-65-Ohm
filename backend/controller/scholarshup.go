...
func CreateScholarship(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", scholarship.ScholarAdminID).First(&scholar_admin); tx.RowsAffected == 0 {
...
func ListScholarships(c *gin.Context) {
	...
	if err := entity.DB().Preload("ScholarAdmin").Preload("ScholarStatus").Preload("ScholarType").Raw("SELECT * FROM scholarships").Find(&scholarships).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
